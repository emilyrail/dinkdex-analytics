"""Live score entry — hot path, optimized for phone."""
...
if st.button("Save round scores", type="primary", use_container_width=True):
    ...
    for _, row in round_df.iterrows():
        ...
        new_a = st.session_state.get(f"score_a_{selected_round}_{mid}")
        new_b = st.session_state.get(f"score_b_{selected_round}_{mid}")

        old_has = pd.notna(old_a) and pd.notna(old_b)
        new_has = new_a is not None and new_b is not None
